...
def process_line(line):
    ...
    if value_type.startswith("enum"):
        value["type"] = "string" if required else [None, "string"]
    elif value_type.startswith("[Money]"):
        value["type"] = "object" if required else [None, "object"]
        value["properties"] = {
            "CurrencyCode": {"type": [None, "string"]},
            "Amount": {"type": [None, "number"]},
        }
    elif value_type in ["integer", "string", "boolean"]:
        value["type"] = value_type if required else [None, value_type]
    else:
        value["type"] = "object" if required else [None, "object"]
        value["properties"] = {}
